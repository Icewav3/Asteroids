...
function draw() {
    ...
    if (debug){
        ...
        text(`Object 2: x=${Math.round(playerShip.position.x)}, y=${Math.round(playerShip.position.y)}, radius=${playerShip.collider.radius}`, 150, 50);
        text(`Distance between centers: ${Math.round(p5.Vector.dist(obj1.position, playerShip.position))}`, 180, 70);
        text(`Sum of radii: ${obj1.collider.radius + playerShip.collider.radius}`, 130, 90);
    }
}
